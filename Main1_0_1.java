...
import static in.codifi.tradehub.ClientWS.PING_INTERVAL;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Matcher m = Pattern.compile("userSession=([^}]+)").matcher(sessionResp.toString());
            ...
            if (RUN_MODIFY_ORDER) {
                ...
                String brokerOrderId = "25120600076067";
                String price = "6.5";
            ...
            if (RUN_EXIT_BRACKET) {
                ...
                String brokerOrderId = "250424000007880";
            ...
            if (RUN_GET_ORDER_HISTORY) {
                ...
                String brokerOrderId = "25120600002107";
            ...
            ClientWS client = new ClientWS(sessionId);
            String invResp = client.invalidateWebSocketSession(USER_ID);
            System.out.println("Invalidate resp: " + invResp);
            String createResp = client.createWebSocketSession(USER_ID);
            System.out.println("Create resp: " + createResp);

            // connect (throws on failure)
            ClientWS.Dispatcher disp = client.connectWS(sessionId, USER_ID);
            BlockingQueue<String> incoming = disp.getCh();

            // start a thread to print all incoming messages
            Thread printer = new Thread(() -> {
                try {
                    while (true) {
                        String ms = incoming.take();
            ...
            // start keepalive
            BlockingQueue<String> kaStatus = new ArrayBlockingQueue<>(10);
            client.KeepAlive(PING_INTERVAL, true, kaStatus);

            // subscribe (matches Go)
            System.out.println("\n*** Market data");
            client.SubscribeMarketData("NSE|26000#NSE|26009");
            Thread.sleep(10_000);
            client.UnsubscribeMarketData("NSE|26000#NSE|26009");

            System.out.println("\n*** Depth data");
            client.SubscribeDepthData("MCX|458305");
            Thread.sleep(10_000);
            client.UnsubscribeDepthData("MCX|458305");

            System.out.println("\n*** Keep alive for 120s to check heart beat");
